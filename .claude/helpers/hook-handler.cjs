...
async function main() {
  ...
  const handlers = {
    ...
    "session-restore": async () => {
      ...
      if (intelligence && intelligence.init) {
        var initResult = await runWithTimeout(function () {
          return intelligence.init();
        }, "intelligence.init()");
    ...
    "session-end": async () => {
      ...
      if (intelligence && intelligence.consolidate) {
        var consResult = await runWithTimeout(function () {
          return intelligence.consolidate();
  ...
  if (command && handlers[command]) {
    ...
    console.log(
      "Usage: hook-handler.cjs <route|pre-bash|post-edit|session-restore|session-end|pre-task|post-task|compact-manual|compact-auto|status|stats>",
    );
  }
}
