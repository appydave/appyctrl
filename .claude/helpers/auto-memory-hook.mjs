  ...
  try {
    ...
    try {
      const cliDistPath = join(
        PROJECT_ROOT,
        "v3/@claude-flow/cli/dist/src/memory/memory-initializer.js",
      );
...
async function doImportAll() {
  ...
  log(
    `Found ${memoryFiles.length} memory files across ${new Set(memoryFiles.map((f) => f.project)).size} projects`,
  );
  ...
  if (!memInit) {
    dim(
      "Cannot vectorize without memory-initializer. Run: cd v3/@claude-flow/cli && npm run build",
    );
    return;
  }
