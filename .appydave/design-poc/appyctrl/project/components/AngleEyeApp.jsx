      ...
      {items.map((it) => (
        <button
          ...
          style={{
            ...
            borderLeft: active === it.id ? "2px solid var(--ac-accent)" : "2px solid transparent",
          }}
          onMouseEnter={(e) => {
            if (active !== it.id) e.currentTarget.style.background = "var(--ac-row-hover)";
          }}
          onMouseLeave={(e) => {
...
function FilterChip({ label, active, accent, onClick }) {
  const bg = active ? (accent ? "var(--ac-accent)" : "var(--ac-row-active)") : "transparent";
  const fg = active && accent ? "var(--brand-brown)" : active ? "var(--ac-fg-1)" : "var(--ac-fg-2)";
  return (
    <button
      onClick={onClick}
      style={{
  ...
  return (
    <button
      ...
      style={{
        ...
        borderLeft: selected ? "3px solid var(--ac-accent)" : "3px solid transparent",
      ...
      onMouseEnter={(e) => {
        if (!selected) e.currentTarget.style.background = "var(--ac-row-hover)";
      }}
      onMouseLeave={(e) => {
...
function ObserverView() {
  ...
  const sessions = [
    ...
    },
    {
      ...
      hash: "728406a5",
      tag: "BUILD",
      note: "/enrich-subtypes 50 build feature",
      ago: "8h ago",
      runtime: "562m",
    },
    {
    ...
    },
    {
      ...
      hash: "184e43ce",
      tag: "BUILD",
      note: "where is this implementation plan written to?",
      ago: "10h ago",
    },
  ];
  ...
  const events = [
    {
      time: "00:41:48",
      kind: "user_prompt",
      text: "Appsupport@signal.com.au is 95% BMAD life cycle, but that's not a useful way of …",
      meta: "↓ 4 tool calls · 6 other",
    },
    {
      time: "00:36:47",
      kind: "user_prompt",
      text: "If you had to go back over everything again, what would you do differently? Hav…",
      meta: "↓1 · 1 tool call · 1 stop · 1 subagent event · 1 other",
    },
    { time: "00:34:28", kind: "user_prompt", text: "server is back", meta: "↓1" },
    {
      time: "00:20:59",
      kind: "user_prompt",
      text: "you should ship deferred code changes, and there's no reason to run 90-second l…",
      meta: "↓1 · 15 tool calls · 1 stop · 1 subagent event · 1 other",
    },
    {
      time: "00:16:08",
      kind: "user_prompt",
      text: "/oco /enrich-subtypes 50 undefined",
      meta: "↓1 · 3 tool calls · 1 stop · 1 subagent event · 1 other",
    },
    {
      time: "00:16:12",
      kind: "user_prompt",
      text: "/oco /enrich-subtypes 50 build skipped",
      meta: "↓1 · 3 tool calls · 2 stops · 1 subagent event · 4 other",
    },
  ];
  ...
  return (
    ...
    >
      ...
      {selWithEvents && <Inspector session={selWithEvents} onClose={() => setSelected(null)} />}
    </div>
  );
}
