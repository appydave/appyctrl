...
function App() {
  ...
  const [apps, setApps] = useAppState([
    {
      ...
      glyph: "◉",
      kind: "local",
      target: "pnpm --filter angle-eye dev",
      port: "4321",
      autostart: true,
      status: "live",
    },
  ...
  const handleSaveApp = (cfg) => {
    if (appModal?.mode === "edit" && appModal.app) {
      ...
      const id =
        (cfg.name || "app").toLowerCase().replace(/[^a-z0-9]+/g, "-") +
        "-" +
  ...
  return (
    ...
    >
      ...
      {showSettings && <SettingsModal tweaks={tweaks} onClose={() => setShowSettings(false)} />}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Polish">
          <TweakToggle
            ...
            value={!!tweaks.ghostWatermark}
            onChange={(v) => setTweak("ghostWatermark", v)}
          />
          <TweakRadio
            label="Density"
            value={tweaks.density || "comfy"}
        ...
        <TweakSection label="Panes">
          ...
          <TweakButton
            label="Open Add Application modal"
            onClick={() => setAppModal({ mode: "add" })}
          />
