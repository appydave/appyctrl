...
function Popover({ open, onClose, anchorRef, children, placement = 'top', width, style }) {
  ...
  const positioned = placement === 'top'
    ? { left: r.left, bottom: window.innerHeight - r.top + 6 }
    : { left: r.left, top: r.bottom + 6 };
  return (
    <div ref={ref} style={{
      position: 'fixed', zIndex: 60, width: width || 'auto',
...
function ModelPickerPopover({ anchorRef, open, onClose, value, onPick }) {
  ...
  return (
    <Popover anchorRef={anchorRef} open={open} onClose={onClose} placement="top" width={420}>
      <div style={{ padding: 12, borderBottom: '1px solid var(--ac-sidebar-border)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ color: 'var(--ac-accent)' }}><IconBolt size={14} /></span>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              color: 'var(--ac-fg-1)', fontFamily: 'var(--font-body)', fontSize: 13,
            }}
          />
        </div>
      </div>
        ...
        {filtered.map(m => (
          <button key={m.id}
            ...
            style={{
              ...
              border: 'none', cursor: 'pointer', color: 'var(--ac-fg-1)',
              fontFamily: 'var(--font-body)', fontSize: 13,
            }}
            onMouseEnter={(e) => e.currentTarget.style.background = 'var(--ac-row-hover)'}
            onMouseLeave={(e) => e.currentTarget.style.background = value === m.id ? 'var(--ac-row-active)' : 'transparent'}
          >
            <Icon size={14} fill="none" sw={1.6}><path d="M12 2l2.5 7L22 11l-6 4.5L18 22l-6-4-6 4 2-6.5L2 11l7.5-2z" /></Icon>
...
function ReasoningPopover({ anchorRef, open, onClose, reasoning, setReasoning, context, setContext }) {
  const reasonings = [
    { id: 'low', label: 'Low' },
  ...
  return (
    <Popover anchorRef={anchorRef} open={open} onClose={onClose} placement="top" width={240}>
      <Section title="Reasoning">
        {reasonings.map(r => (
          <PopRow key={r.id} active={reasoning === r.id} onClick={() => setReasoning(r.id)} label={r.label} />
        ))}
      </Section>
      <div style={{ borderTop: '1px solid var(--ac-sidebar-border)' }} />
      <Section title="Context Window">
        {contexts.map(c => (
          <PopRow key={c.id} active={context === c.id} onClick={() => setContext(c.id)} label={c.label} />
        ))}
      </Section>
    </Popover>
  );
...
function Section({ title, children }) {
  return (
    <div style={{ padding: '12px 8px 8px' }}>
      <div style={{
        padding: '0 10px 6px', color: 'var(--ac-fg-3)',
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        width: '100%', padding: sub ? '8px 10px' : '6px 10px',
        borderRadius: 6, border: 'none', background: 'transparent',
        cursor: 'pointer', color: 'var(--ac-fg-1)',
...
function AccessPopover({ anchorRef, open, onClose, value, onPick }) {
  const opts = [
    { id: 'supervised',  icon: <IconLock size={13} />, label: 'Supervised', sub: 'Ask before commands and file changes.' },
    { id: 'auto',        icon: <Icon size={13} sw={1.7}><path d="M12 20h9" /><path d="M16.5 3.5a2.12 2.12 0 1 1 3 3L7 19l-4 1 1-4z" /></Icon>, label: 'Auto-accept edits', sub: 'Auto-approve edits, ask before other actions.' },
    { id: 'full',        icon: <IconLock size={13} />, label: 'Full access', sub: 'Allow commands and edits without prompts.' },
  ];
  return (
      ...
      <div style={{ padding: 6 }}>
        {opts.map(o => (
          <button key={o.id}
            ...
            onMouseEnter={(e) => e.currentTarget.style.background = 'var(--ac-row-hover)'}
            onMouseLeave={(e) => e.currentTarget.style.background = value === o.id ? 'var(--ac-row-active)' : 'transparent'}
            style={{
              display: 'flex', alignItems: 'flex-start', gap: 10,
              width: '100%', padding: '10px 12px', borderRadius: 6,
              background: value === o.id ? 'var(--ac-row-active)' : 'transparent',
              border: 'none', cursor: 'pointer', color: 'var(--ac-fg-1)', textAlign: 'left',
            }}>
  ...
  return (
    <div style={{
      ...
      <svg width="26" height="26" viewBox="0 0 26 26" style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx="13" cy="13" r={r} fill="none" stroke="var(--ac-fg-2)" strokeWidth="2"
          strokeDasharray={`${dash} ${c}`} strokeLinecap="round" />
      </svg>
      <span style={{
        position: 'absolute', fontSize: 9, color: 'var(--ac-fg-2)',
