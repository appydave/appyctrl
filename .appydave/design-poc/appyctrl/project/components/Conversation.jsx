    ...
    >
      ...
      <div style={{ maxWidth: 820, marginLeft: "auto", marginRight: "auto", position: "relative" }}>
        <UserCard>
          ...
          <MdLine>
            The app name is now "AppyDave" but the T3 wordmark SVG and cold dark colours
          </MdLine>
          <MdLine>
            remain. Replace the wordmark in the sidebar header, apply AppyDave warm dark
          </MdLine>
          <MdLine>colour tokens, and swap the boot/favicon icons once assets are available.</MdLine>

          <MdHeading>## Resources</MdHeading>
          <MdLine>
            - Design plan: <Mono>.appydave/docs/design-plan.md</Mono> (items 1 + 2)
          </MdLine>
          <MdLine>
            - Brand tokens: invoke <Mono>brand-dave:brand</Mono> skill
          </MdLine>
          <MdLine>
            - Wordmark location: <Mono>apps/web/src/components/Sidebar.tsx</Mono> ~line 2404
          </MdLine>
          <MdLine indent={1}>(T3Wordmark SVG + hardcoded "Code" text)</MdLine>
          <MdLine>
            - CSS token location: <Mono>apps/web/src/index.css</Mono> (dark mode block ~line 118)
          </MdLine>
          <MdLine>
            - Icon assets: <Mono>apps/desktop/resources/</Mono> + <Mono>apps/web/public/</Mono>
          </MdLine>
          <MdLine>
            - Coding standards: <Mono>.appydave/docs/coding-standards.md</Mono>
          </MdLine>

          <MdHeading>## What To Do</MdHeading>
          <MdLine>
            1. Create <Mono>apps/web/src/appydave/AppyWordmark.tsx</Mono>
          </MdLine>
          <MdLine indent={1}>
            — "Appy" in gold #ccba9d, "Dave" in yellow #ffde59, Oswald font
          </MdLine>
          <MdLine>
            2. Create <Mono>apps/web/src/appydave/brand.css</Mono>
          </MdLine>
          <MdLine indent={1}>
            — Override dark mode CSS tokens (chrome #1a1515, surface #25201e,
          </MdLine>
          <MdLine indent={1}>&nbsp;&nbsp;primary #ffde59, accent #ccba9d, text #faf5ec)</MdLine>
          <MdLine>
            3. Seam edit <Mono>main.tsx</Mono> — add import <Mono>'./appydave/brand.css'</Mono>{" "}
            after index.css
          </MdLine>
          <MdLine>
            4. Seam edit <Mono>Sidebar.tsx</Mono> — replace <Mono>{"<T3Wordmark />"}</Mono> + "Code"
            with <Mono>{"<AppyWordmark />"}</Mono>
          </MdLine>
          <MdLine>
            5. Icons — skip until AppyDave logo assets provided (icns/ico/png/favicons)
          </MdLine>
          <MdLine>
            6. <Mono>bun fmt && bun lint && bun typecheck</Mono> — must pass
          </MdLine>
          <MdLine>
            7. <Mono>commit feat(appydave): apply AppyDave wordmark and colour tokens</Mono>
          </MdLine>

          <MdHeading>## Key constraints</MdHeading>
          <MdLine>- App runs dark by default — do NOT flip to cream light theme</MdLine>
          <MdLine>
            - <Mono>Sidebar.tsx</Mono> is an upstream file — minimal edit only (swap the component,
            nothing else)
          </MdLine>
          <MdLine>
            - All new code in <Mono>apps/web/src/appydave/</Mono> (never conflict on rebase)
          </MdLine>
        </UserCard>
  ...
  const sendBg =
    tweaks.theme === "cold-t3"
      ...
      : sendActive
        ...
        : "var(--ac-send-idle-bg)";
  const sendFg =
    tweaks.theme === "cold-t3" ? "#ffffff" : sendActive ? "var(--brand-brown)" : "var(--ac-fg-2)";
        ...
        >
          ...
          <span ref={reasonRef} style={{ display: "inline-flex" }}>
            <ComposerPill onClick={() => setOpenReason((v) => !v)}>
              <span style={{ color: "var(--ac-fg-2)" }}>
                {reasoningCap} · {contextSize}
              </span>
              <IconChevronDown size={11} />
            </ComposerPill>
          </span>
  ...
  return (
    <span
      style={{
        width: 1,
        height: 14,
        background: "var(--ac-composer-sep)",
        margin: "0 2px",
      }}
    />
  );
