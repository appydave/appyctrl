    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <div style={{
          ...
          fontSize: 13, letterSpacing: '0.04em', color: '#e6e6e6',
        }}>T3.code</div>
      </div>
    );
  }
...
function ThreadRow({ title, time, working, active, onClick }) {
  return (
    ...
    >
      {working && (
        <span className="ac-working-pill" style={{
          display: 'inline-flex', alignItems: 'center', gap: 5,
          fontSize: 11, fontWeight: 500,
          flexShrink: 0,
        }}>
          <span className="ac-working-dot" style={{
      ...
      <span style={{
        fontSize: 11, color: 'var(--ac-fg-3)', fontVariantNumeric: 'tabular-nums',
        flexShrink: 0,
      }}>{time}</span>
    </button>
  );
...
function ProjectGroup({ project, openSort, setOpenSort, activeThread, setActiveThread }) {
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      ...
      >
        <span style={{ display: 'inline-flex', transform: expanded ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 120ms var(--ease-out)', color: 'var(--ac-fg-2)' }}>
          <IconChevronRight size={12} />
        </span>
        <IconFolder size={14} style={{ color: 'var(--ac-fg-2)' }} />
        <span>{project.name}</span>
      </button>
      {expanded && project.threads.map(t => (
        <ThreadRow
          key={t.id}
          title={t.title}
          time={t.time}
...
function SortPopover({ onClose }) {
  return (
    ...
    >
      <SortGroup title="Sort projects" items={['Last user message', 'Created at', 'Manual']} active="Last user message" />
      <SortGroup title="Sort threads" items={['Last user message', 'Created at']} active="Last user message" />
      <div style={{ height: 1, background: 'var(--ac-popover-border)', margin: '4px 6px' }} />
      <SortGroup title="Group projects" items={['Group by repository', 'Group by repository path', 'Keep separate']} active="Group by repository" />
    </div>
  );
}
...
function SortGroup({ title, items, active }) {
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      <div style={{
        padding: '6px 10px 4px', fontSize: 11, color: 'var(--ac-fg-3)',
        fontWeight: 500,
      }}>{title}</div>
      {items.map(it => (
        <button
  ...
  return (
    ...
    >
      ...
      <span
        role="button"
        title="Configure"
        onClick={(e) => { e.stopPropagation(); onConfigure && onConfigure(app); }}
...
function Sidebar({ tweaks, activeThread, setActiveThread, mode, setMode, activeApp, setActiveApp, apps, onAddApplication, onConfigureApplication, onOpenSettings }) {
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '10px 12px 6px' }}>
        <div className="ac-search-row" style={{
          display: 'flex', alignItems: 'center', gap: 8,
          padding: '6px 10px',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px' }}>
        ...
        <div style={{
          ...
          <span className="ac-label" style={{
            fontFamily: 'var(--font-heading)', textTransform: 'uppercase',
            letterSpacing: '0.08em', fontSize: 10, color: 'var(--ac-fg-3)',
            fontWeight: 500,
          }}>Projects</span>
        ...
        {projects.map(p => (
          <ProjectGroup
            ...
            setActiveThread={(id) => { setMode('projects'); setActiveThread(id); }}
          />
        ))}
        ...
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '14px 8px 6px', marginTop: 6,
          borderTop: '1px solid var(--ac-sidebar-border)',
        }}>
          <span className="ac-label" style={{
            fontFamily: 'var(--font-heading)', textTransform: 'uppercase',
            letterSpacing: '0.08em', fontSize: 10, color: 'var(--ac-fg-3)',
            fontWeight: 500, paddingTop: 8,
          }}>Applications</span>
        ...
        {(apps || []).map(a => (
          <AppRow
            ...
            onClick={() => { setMode('apps'); setActiveApp(a.id); }}
