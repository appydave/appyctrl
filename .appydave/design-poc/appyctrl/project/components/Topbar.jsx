  ...
  return (
    <button
      ...
      style={{
        ...
        gap: 6,
        height: 28,
        padding: hasCaret ? "0 4px 0 10px" : "0 10px",
      ...
      {open && (
        ...
        >
          {items.map((it, i) => (
            ...
            >
              <span style={{ color: it.iconColor || "var(--ac-fg-2)", display: "inline-flex" }}>
                {it.icon}
              </span>
...
function Topbar({ tweaks, onAddAction, onToggleTerminal, onToggleDiff, showTerminal, showDiff }) {
  return (
    ...
    >
      ...
      >
        <span style={{ color: "var(--ac-fg-2)", overflow: "hidden", textOverflow: "ellipsis" }}>
          AppyDave wordmark and colors
        </span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <ChromeButton onClick={onAddAction} leftIcon={<IconPlus size={13} sw={2.2} />}>
          Add action
        </ChromeButton>
        ...
        <SplitButton
          ...
          items={[
            {
              icon: <IconVSCode size={15} />,
              iconColor: tweaks.theme === "cold-t3" ? "#3b9eff" : "var(--brand-gold)",
              label: "VS Code",
              shortcut: "⌘O",
            },
